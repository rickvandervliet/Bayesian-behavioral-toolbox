...
import queue
import logging
import OneRateModel_NoiseNonHierarchical_MvL_OOC as model
...
class StartWindow(tk.Frame):
    def __init__(self, master):
        ...
        self.new_model_button.grid(row=0,pady=4)
...
class AdvancedSettingsWindow(tk.Frame):
    def __init__(self,master):
        ...
        self.return_button = tk.Button(self,text='OK', command=lambda:master.switch_frame(StartWindow))
...
class HierarchicalWindow(tk.Frame):
    def __init__(self,master):
        ...
        methods = ["Gamma","NonCentered", "Ratio"]
        master.method_var.set(methods[0])
        self.method_label = tk.Label(self,text='Method for calculating noise:')
        self.method_label.grid(column=0,row=1,padx=2,pady=4)
        self.method_menu = ttk.Combobox(self, textvariable=master.method_var, values=methods)
        ...
        self.return_button = tk.Button(self,text='Back', command=lambda:master.switch_frame(StartWindow))
    ...
    def run_hierarchical(self,master):
        ...
        ssm = model.StateSpace(data,method=self.method_var.get())
...
class NonHierarchicalWindow(tk.Frame):
    def __init__(self,master):
        ...
        self.iter_entry.grid(column=1,row=0,padx=2,pady=4)
        self.save_img_button = tk.Button(self,text='Save images', command=lambda:master.switch_frame(ImageSelectWindow))
        self.save_img_button.grid(column=0,row=1,padx=2,pady=4)
        self.run_button = tk.Button(self,text='Run', command=lambda:self.run_thread(master))
        self.run_button.grid(column=1,row=2,padx=2,pady=4)
        self.return_button = tk.Button(self,text='Back', command=lambda:master.switch_frame(StartWindow))
        self.return_button.grid(column=0,row=2,padx=2,pady=4)
        # self.out = ThreadSafeText(self)
        # self.out.grid(column=0,row=2,columnspan=2)
        # redir = IORedirector(self.out)
        # sys.stdout = redir
        # sys.stderr = redir
        # logging.basicConfig(stream=redir,format='%(message)s')
...
class ImageSelectWindow(tk.Frame):
    def __init__(self,master):
        ...
        if master.shared_data["hierarchical"].get() == "Non Hierarchical":
            ...
            if master.method_var.get() == "Ratio":
            ...
            elif master.method_var.get() == "Gamma":
            ...
            elif master.method_var.get() == "NonCentered":
